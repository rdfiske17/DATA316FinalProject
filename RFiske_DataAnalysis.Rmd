...
GDPlineF <- function(x){coef(mod.5)[1]*log(x) + coef(mod.5)[2]*mean(log(life_data$ratio_below_povline+0.01)) + coef(mod.5)[3]*mean(life_data$schooling_years) + coef(mod.5)[4]}

GDPlineM <- function(x){coef(mod.5)[1]*log(x) + coef(mod.5)[2]*mean(log(life_data$ratio_below_povline+0.01)) + coef(mod.5)[3]*mean(life_data$schooling_years) + coef(mod.5)[5]}

ratiolineF <- function(x){coef(mod.5)[1]*mean(life_data$gdp_per_capita) + coef(mod.5)[2]*log(x) + coef(mod.5)[3]*mean(life_data$schooling_years) + coef(mod.5)[4]}

ratiolineM <- function(x){coef(mod.5)[1]*mean(life_data$gdp_per_capita) + coef(mod.5)[2]*log(x) + coef(mod.5)[3]*mean(life_data$schooling_years) + coef(mod.5)[5]}

schoollineF <- function(x){coef(mod.5)[1]*mean(life_data$gdp_per_capita) + coef(mod.5)[2]*mean(log(life_data$ratio_below_povline+0.01)) + coef(mod.5)[3]*x + coef(mod.5)[4]}

schoollineM <- function(x){coef(mod.5)[1]*mean(life_data$gdp_per_capita) + coef(mod.5)[2]*mean(log(life_data$ratio_below_povline+0.01)) + coef(mod.5)[3]*x + coef(mod.5)[5]}

#Visualize the lines with the data
ggplot(life_data, aes(x = log(gdp_per_capita), y = life_expectancy)) +
...
#Source https://www.worlddata.info/life-expectancy.php
```
